...
from ntk import get_transform_matrix, get_same_class_relative_ratio, kernel_regression
from jax_models import MLPNet, WideResnet, CNN


def get_test_accuracy_multi(test_outputs, test_targets):
    total = test_outputs.shape[0]
    pred = np.argmax(test_outputs, axis=1)
    # (n, 1)
    gold = np.argmax(test_targets, axis=1)
    # (n, 1)
    print('pred', pred)
    print('gold', gold)
...
def run_ntk_experiments():
    ...
    config = {'sample_size': 50000, 'seed': 66, 'dataset': 'CIFAR10', 'input_size': 3 * 32 * 32 + 1,
              'hidden_size': 4096, 'epoch_num': 500, 'simple_train_batch_size': 50, 'simple_test_batch_size': 50,
              'lr': 0.5, 'dir_path': '/path/to/working/dir', 'one-hot': True}
    ...
    total_inputs_train, total_outputs_train = process_data_numpy_multi(train_data, 'normal')
    total_inputs_test, total_outputs_test = process_data_numpy_multi(test_data, 'normal')
