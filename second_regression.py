...
def get_pair_predictions(train_train_features, test_train_features, train_train_labels, epsilon=0.01):
    ...
    # (b*n, 1)
    print('test train features pred', test_train_features_pred)
    train_train_features_pred = np.dot(train_train_features, weights)
    print('train train features pred', train_train_features_pred)
...
def run_ntl_experiments():
    ...
    config = {'sample_size': 10000, 'seed': 66, 'dataset': 'CIFAR10', 'input_size': 3 * 32 * 32 + 1,
              ...
              'dir_path': '/path/to/working/dir', 'label_ratio': 0.1, 'ntl_version': 'V8',
              'train': True}
    ...
    train_label_products = (np.dot(total_outputs_train, np.transpose(total_outputs_train)) >= 0)
    test_label_products = (np.dot(total_outputs_test, np.transpose(total_outputs_train)) >= 0)
